 ...
 * runtime import graph stops at this file and never reaches the answer bank: a
 * structural guarantee rather than one the bundler happens to provide.
 *
 * `wheel.ts` re-exports everything here, so the reducer and its tests still
 ...
 * `FINDS_PER_TURN` is three, so a good turn is three spins, and two bad wedges
 * in twenty-four took better than a fifth of all turns to nothing
 * (1 - (22/24)^3 = 21%). Two in thirty-six takes 16%, frightening without being
 * the thing the game is mostly about, and it was the second complaint about
 * this wheel after the size of its numbers.
 */
 ...
 * long it takes, the curve the board eases along) falls out of that one number,
 * which is why it is the only one tuned by feel.
 *
 * Constant deceleration also gives the animation its shape exactly: distance
 * is `2t - t^2` in normalised time, which is a quadratic ease-out, which is the
 ...
 */
export const SPIN_DRAG = 0.000145;

/**
 * How much wheel a finger's rotation is worth.
 *
 * The hand and the wheel are *not* the same speed here, and pretending they
 * were is the bug this model was rewritten for. The hub sits a full radius
 * below the bottom of the frame (see `HUB_Y` in `WheelBoard.tsx`), so a finger
 * on the rim turns about a pivot most of a screen away, and a swipe crossing
 * the entire phone is barely sixty degrees of wheel. Measured: a hard 300px
 * flick over 120ms is 0.35 deg/ms, against an old speed floor of 0.584. Every
 * flick ever thrown in this game clamped to `SPIN_MIN_TRAVEL`, in both
 * directions, on every device. That is why the wheel did not care how hard you
 * threw it, and why it never once stopped off a wedge's midpoint: 102 is a
 * whole number of wedges, so `rest` could never pick up a fraction and the
 * landing cycled 6, 0, 6, 0 for ever.
 *
 * The gain is the one number that couples hand to wheel, and it is chosen so
 * that the two ends of a *real* flick are the two ends of the wheel's range:
 * about 170px of thumb in 120ms is the gentlest throw the wheel accepts, and
 * about 380px is as hard as it is worth throwing. Everything between is the
 * square law, untouched.
 */
export const FLICK_GAIN = 1.8;

/**
 * How far a spin carries, in wedges, at the gentlest accepted flick and the
 * hardest useful one.
 *
 * The floor is a *gate* now rather than a clamp, and that difference is the
 * anti-cheat. A clamp meant every under-strength release became the same throw,
 * measured from a `rest` the player can see: a known distance from a known
 * place, which is aiming with extra steps. A gate means a release too slow to
 * be a throw is not a throw at all. The move is refused, the wheel is left
 * exactly where the hand put it, and the player is asked to flick it harder.
 * Nothing deterministic ever resolves, so there is nothing to aim.
 *
 * The ceiling is deliberately not a whole number of wedges. It is the one clamp
 * a player still meets regularly, since any hard flick lands on it, and a whole
 * number there would put every hard throw back on a midpoint, the artefact this
 * rewrite exists to kill.
 *
 * 1.5 turns at the gate, a little over 5 at the ceiling, which is 2.7 seconds
 * to 5.0. Constant friction ties those together: `t = sqrt(2s/a)`, so the
 * distance range and the duration range are one choice, not two.
 */
export const SPIN_MIN_TRAVEL = 54;
export const SPIN_MAX_TRAVEL = 181.5;

/** The release speed, in degrees of rotation per millisecond, that carries the
    wheel exactly `wedges` wedges: `v = sqrt(2 * a * s)`. Wheel speed, not finger
    speed; divide by `FLICK_GAIN` for what a hand has to do. */
const speedFor = (wedges: number) =>
  Math.sqrt(2 * SPIN_DRAG * wedges * WEDGE_ARC);

/**
 * The two ends of a flick, in degrees per millisecond **of finger**.
 *
 * Derived rather than chosen, so the gate on speed and the clamps on distance
 * remain the same two facts and cannot drift apart. These are what a pointer
 * trail actually reports, which is why the gain is divided out here: every
 * caller comparing against a measured release wants finger units.
 */
export const SPIN_MIN_SPEED = speedFor(SPIN_MIN_TRAVEL) / FLICK_GAIN;
export const SPIN_MAX_SPEED = speedFor(SPIN_MAX_TRAVEL) / FLICK_GAIN;
 ...
 *
 * The board hands this the distance it is *actually drawing*, which a drag can
 * have pushed a little past the ceiling, so the clamp stays here.
 */
...
/**
 * Whether a release was a throw at all.
 *
 * Separate from `spinThrow` because the two answers have different audiences:
 * the reducer needs to *refuse* a nudge, and the board would like to know
 * before it spends a round trip finding out. One threshold, both callers.
 *
 * A hostile or broken client is covered by the same line: NaN is not a throw.
 */
export function isThrow(velocity: number): boolean {
  return Number.isFinite(velocity) && Math.abs(velocity) >= SPIN_MIN_SPEED;
}

/**
 * The throw a release at `velocity` makes, where velocity is signed degrees of
 * rotation per millisecond of *finger* at the moment it left the rim, or
 * `null` if that was a nudge rather than a throw.
 *
 ...
 * Aiming is stopped twice over. The landing is measured from where the wheel
 * *stopped last time* rather than from wherever a drag left the rim, so lining
 * the rim up buys nothing; and the gate means the one throw whose distance a
 * player could know exactly, the weakest one, never resolves at all.
 */
export function spinThrow(velocity: number): Throw | null {
  if (!isThrow(velocity)) return null;
  const speed = Math.abs(velocity) * FLICK_GAIN;
  // s = v^2 / 2a, in degrees, then in wedges. Deliberately *not* rounded: a
  ...
  // is the one that came up. Only the ceiling clamps: the floor is the gate
  // above, and a throw that got here has already cleared it.
  const wedges = (speed * speed) / (2 * SPIN_DRAG * WEDGE_ARC);
  const travel = Math.min(wedges, SPIN_MAX_TRAVEL);
  return { travel: velocity < 0 ? -travel : travel, ms: spinMs(travel) };
}
 ...
 *
 * The minus sign is worth reading twice. The wedges are numbered clockwise from
 * twelve o'clock (see `sectorPath`), so turning the disc clockwise brings the
 * wedge *before* the current one under the pointer. Rotation and index run
 * opposite ways, and every conversion between them goes through here.
 */
 ...
 * and goes to the higher wedge. Arbitrary, but it has to go somewhere, and the
 * flapper is standing on the peg between them either way.
 */
 ...
 *
 * In practice only the cash wedges reach it: a 10 deg slice has no room for a
 ...
 * whatever ICU data a given Node build happens to ship. A reducer that formats
 * differently on the server than in a test is not a pure reducer.
 */
