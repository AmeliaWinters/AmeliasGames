 ...
 * before the player commits, and how a bid reads as English: real logic,
...
export interface Showdown {
  ...
  actual: number;
  /**
   * Who gave up a die, or null, which only happens on a spot-on call that was
   * right, the one call in the game that costs nobody anything.
   */
  loser: number | null;
  /**
   ...
   * place hands are ever public, and the whole point of the game: a call you
   * cannot see the result of teaches nobody anything.
   */
  hands: number[][];
...
export interface LdState {
   ...
   * the table. The sequence is public, it was all said out loud, and it is most
   * of what a call is reasoned from: who climbed eagerly and who was dragged.
   * Cleared when the next round is rolled, and deliberately kept through the
   * reveal so the showdown reads against the bidding that led to it.
   */
  history: Bid[];
 ...
 *
 * Rolling is the one thing here that happens out of turn, since everyone's
 * dice hit the table at once, so it is a second predicate beside `turn` and
 * `canAct` is the union of the two. Bidding controls gate on `turn`, not on
 * `canAct`, or every player gets a bid box at the top of a round.
 */
 ...
 * hand is all `HIDDEN_FACE`, which is not a face and counts as nothing.
 */
