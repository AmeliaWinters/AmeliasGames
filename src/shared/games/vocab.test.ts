...
describe('asking it the other way round', () => {
   ...
   * secret on a `hear` is the *meaning*, exactly as on a `pick`.
   */
...
describe('hints', () => {
  ...
  it('runs out, and says so', () => {
    ...
    for (let spent = 0; spent < HINT_ALLOWANCE; spent++) {
      // Even a table of experts gets a listening round now, and there is
      // nothing to hint at on one: the answer is not a blank box. So walk to
      // the next typed round before spending.
      while (askIn(state.round, 0) !== 'say') {
...
describe('what each client is told', () => {
   ...
   * marked `retry` is never requeued, so a line still saying "it'll be back"
   * there would be the status bar lying about the rules on the one screen that
   * explains them.
   */
