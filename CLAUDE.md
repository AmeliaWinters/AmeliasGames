...
`winner(state)` is a third and it is neither of those. It is decoration -- the
end screen draws that seat's figure, name and characters -- and it is asked
only of a finished game. It is required rather than optional so that a game
with nobody to name says so: `null` is a real answer, and it covers a draw, a
tie at the top and Drill, which is played alone. Nothing is decided from it.

## When a file has outgrown itself

Line count on its own decides nothing here. The two largest files in the repo
are `chainWords.ts` at 82,000 lines and `words.ts` at 54,000, and both are
correct as they are: they are generated, they are regenerated by a script, and
splitting them would make them worse. `waifuRoster.ts`, `wardrobe.ts` and
`vocabPhrases.ts` are the same bargain by hand -- one flat table, one thing per
row. **A file that holds one kind of thing is finished however long it is.**

What actually goes wrong is a file holding *three* kinds of thing, where the
one you came for is somewhere in the middle. Every split already made here was
that, and each was cut along a seam the file had already grown:

- **A board is three or four screens and a dozen pictures.** `VocabBoard.tsx`
  was 1,372 lines: the copy, the components, the setup screen, the end screen
  and the round. It is `vocab/copy.ts`, `vocab/parts.tsx`, `vocab/Setup.tsx`,
  `vocab/Over.tsx` and 543 lines of board. Battleships, Word Chain and the
  Wheel are cut the same way, and the shape is always the same: a `copy` or
  `geometry` module of things that are not components, a `parts` module of
  components that are pure functions of their props, a file per whole screen,
  and the board itself keeping the hooks. **The hooks stay on the board.** A
  screen extracted from behind an early return is drawn after the hooks have
  run, so it takes what it needs as props and never reaches for a clock of its
  own -- two countdowns disagreeing by a frame is a bug that only shows up on
  somebody else's phone.
- **A reducer is a definition and the machinery under it.** `vocab.ts` and
  `wordChain.ts` are now the `GameDefinition` object and nothing else, with the
  dealing, drawing, settling and redacting in `vocabRounds.ts` and
  `wordChainTurns.ts`. The moved half is on the *same* side of the bundle
  boundary as the reducer -- it reaches the dictionary, so nothing on the
  client may import it either, and each of those files says so at the top.
  `wheel.ts` gave up its answer bank to `wheelPuzzles.ts` for the same reason
  said the other way round: that file is the secret, and it is easier to keep
  an eye on when it is the whole file.
- **A display module is rules, shapes and derivations.** `vocabDisplay.ts` was
  1,439 lines of all three and is now the door: it re-exports `vocabRules.ts`
  (the numbers the game is tuned by) and `vocabShapes.ts` (what a round is made
  of) and keeps the questions you ask of a live state. `profile.ts` is a
  28-line door over four. **Keep the door.** Every board and the reducer already
  name the old path, the boundary comment on it is load-bearing, and one import
  path is one boundary to keep honest instead of four.

Two rules that are not negotiable when you cut:

**A stylesheet is cut along its own banners and imported adjacently.** CSS is
decided by source order, so `index.css` *is* the cascade. `picker.css`,
`controls.css` and `profile.css` were split at the `/* Section */` banners
already written in them, and the pieces are imported next to each other in the
old file's position, so concatenating them reproduces the original rule for
rule. Check that literally -- `cat a.css b.css | diff - old.css` -- before
believing it. Nothing had to be re-checked in a browser because nothing moved.

**Splitting must not change behaviour, and you have to show it did not.**
`npm run typecheck` and `npm test` both, before and after, and the counts have
to match: the whole value of a mechanical cut is that it is mechanical. A split
that changes a persisted shape or the meaning of a stored field is not a split,
it is a change, and it needs the `SNAPSHOT_VERSION` bump described above.

And the failure mode worth naming, because it is the one that produced this
section: nothing in this repo fails when a file grows. There is no line budget
and no test for it, deliberately -- a number would nag at the generated tables
and say nothing about the file that is actually hard to read. So it is a
judgement, and the question to ask before adding to a long file is not "how
long is this" but **"is the thing I am adding the same kind of thing as what is
already here?"** If it is not, it wants a neighbour, and the neighbour is
cheapest to make now rather than four hundred lines later.
