 ...
 * things per word that no single public source carries together — is it a
 * word, how common is it, and what does it mean in English — and stitching
 * them is a job for a script that runs once, not for a reducer.
 *
 * Why each source:
 *
 ...
 *   actually say" filter, which a dictionary alone will not do — `words.ts`
 *   happily contains `aal` and `abb`.
...
const SOURCES: Record<string, { url: string; note?: string }> = {
  ...
  'en_50k.txt': { url: 'https://raw.githubusercontent.com/hermitdave/FrequencyWords/master/content/2018/en/en_50k.txt' },
...
/** Grammatical furniture and register markers — not words to make anyone chain on. */
...
function firstGloss(senses: string): string {
  for (const raw of senses.split('/')) {
    ...
    // EDICT parks loanword etymologies at the end of a gloss —
...
function emit(pl: PlWord[], ja: JaWord[], en: string[]): string {
  ...
  return `/**
 * Word Chain's three word lists. GENERATED — do not edit by hand.
 *
 *   npm run build:wordchain
 *
